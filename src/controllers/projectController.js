...
export const createProject = async (req, res) => {
  try {
    const { title, short_description, description, category, link, repoUrl, techStack, additionalTools, featured, order } = req.body;
...
export const updateProject = async (req, res) => {
  try {
    ...
    // Get the existing project to preserve current images if no new ones are provided
    ...
    // If new files were uploaded, add them to Cloudinary
    if (req.files && req.files.length > 0) {
      for (const file of req.files) {
        const { url, publicId } = await uploadToCloudinary(file.buffer);
        images.push({ url, publicId, alt: file.originalname });
    ...
    if (Array.isArray(bodyImages) && bodyImages.length > 0) {
      ...
      images = urlImages; // Replace existing images with the provided URLs
    }
